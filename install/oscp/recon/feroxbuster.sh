...
feroxbuster --url http://$1 -o ferox-extended-$1.txt --add-slash --extract-links -x php htm html json js xml asp aspx txt bak sh yml py pl
...
feroxbuster -w /usr/share/dirbuster/wordlists/directory-list-lowercase-2.3-medium.txt --url http://$1 -o ferox-extended-m-$1.txt --add-slash --extract-links -x php htm html json js xml asp aspx txt bak sh yml py pl
